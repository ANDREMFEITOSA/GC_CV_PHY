...
def plotar_graficos(n_bars, colors_name, heights):
    df = pd.DataFrame([heights], index = [colors_name])
    fig, ax = plt.subplots(1, 2, figsize=(5, 5))
    g = ax[0].bar(np.array(colors_name), np.array(heights), width=0.25, color=np.array(colors_name))
    ax[0].grid()
    ax[0].set_title('Bar\'s Height x Color', fontsize=20)
    ax[0].set_ylabel('Height (cm)', fontsize=14)
    ...
    list_g = []
    for i in range(n_bars):
        list_g.append(ax[1].bar('Stack', heights[i], width=0.10, bottom=bottom, align='center', color=colors_name[i]))
        bottom += heights[i] 
    for i in range(n_bars):
        ax[1].bar_label(list_g[i], fmt='{:,.1f}', label_type='center', padding = 3)
    ax[1].grid()
