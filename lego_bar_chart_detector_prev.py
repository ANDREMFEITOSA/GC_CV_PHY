...
#ip = "https:/192.168.81.104:8080/video"
...
if n > 0:
    for i in range(n):
        ...
        list_average_rgb_.append([round((list_lower_rgb[i][0]+list_upper_rgb[i][0])*255/2), round((list_lower_rgb[i][1]+list_upper_rgb[i][1])*255/2), round((list_lower_rgb[i][2]+list_upper_rgb[i][2])*255/2)])
        ...
        list_bar_color_name_.append(color_name.get_color_name((list_average_rgb_[i][0], list_average_rgb_[i][1], list_average_rgb_[i][2])))
            ...
            if captura_ok:
                ...
                for i in range(n):
                    if list_bbox[i] is not None:
                        ...
                        r, g, b = webcolors.name_to_rgb(list_bar_color_name_[i])
                        # frame = cv2.rectangle(frame, (x1, y1), (x2, y2), (list_average_rgb[i][0]*255, list_average_rgb[i][1]*255, list_average_rgb[i][2]*255, 2))
                        # frame = cv2.rectangle(frame, (x1, y1), (x2, y2), (list_average_rgb_[i][0], list_average_rgb_[i][1], list_average_rgb_[i][2], 2))
                        ...
                        cv2.putText(frame, "{}".format(round(object_height - 0.18, 1)), (int(x1), int(y1 - 15)),
                                        ...
                                        # cv2.FONT_HERSHEY_PLAIN, 1, (list_average_rgb_[i][0], list_average_rgb_[i][1], list_average_rgb_[i][2], 2))
                                        cv2.FONT_HERSHEY_PLAIN, 1, (b, g, r, 2))
                print('Height: ', list_bar_h)
                print('Average RGB: ', list_average_rgb_)
                print('Average RGB Color name:', list_bar_color_name_)
                print('RGB red: ', webcolors.name_to_rgb('red'))
                print('RGB limegreen: ', webcolors.name_to_rgb('limegreen'))
                print('RGB turquoise: ', webcolors.name_to_rgb('turquoise'))
                print('RGB sandybrown: ', webcolors.name_to_rgb('sandybrown'))
